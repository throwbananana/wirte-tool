...
def _resolve_data_dir():
    """Resolve the default runtime data directory path."""
    return get_app_paths().runtime_data_dir
...
示例:
  ...
  python analyze_events.py --data-dir /path   # 指定数据目录
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Path to data directory (runtime_data, sample_data, or legacy writer_data)",
    )
    parser.add_argument(
        "--file",
        type=str,
    ...
    if args.file:
        ...
        events_path = (data_dir / "school_events.json") if args.data_dir else get_app_paths().default_events_file()
    ...
    if args.gui:
        ...
        report_path = get_app_paths().logs_dir() / "analyze_events_report.txt"
